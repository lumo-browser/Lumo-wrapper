...
const getStatusBadge = (status: string): { icon: string; color: string } => {
  switch (status) {
    case 'connected':
      return { icon: '✓', color: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' };
    case 'disconnected':
      return { icon: '⊘', color: 'bg-gray-100 dark:bg-gray-900/30 text-gray-700 dark:text-gray-300' };
    case 'error':
      return { icon: '!', color: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300' };
    default:
      return { icon: '?', color: 'bg-gray-100 dark:bg-gray-900/30 text-gray-700 dark:text-gray-300' };
  }
};
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3 flex-1">
              <div className={clsx('w-8 h-8 rounded-full flex items-center justify-center text-sm', badge.color)}>
                {badge.icon}
              </div>
